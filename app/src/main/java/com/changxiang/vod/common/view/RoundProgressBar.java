...
/**
 * Created by 15976 on 2016/12/15.
 */

public class RoundProgressBar extends View {

    private Paint paint;//画笔
    private int roundColor;// 圆环进度的颜色
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (style) {
            case STROKE:{
                ...
                canvas.drawArc(oval, 0, 360.0f * progress / max, false, paint);  //根据进度画圆弧
                break;
            }
            case FILL:{
                paint.setStyle(Paint.Style.FILL_AND_STROKE);
                if(progress !=0)
                    canvas.drawArc(oval, 0, 360.0f * progress / max, true, paint);  //根据进度画圆弧
                break;
            }
        }
    }
