...
import com.changxiang.vod.module.musicInfo.ILrcStateContain;
import com.changxiang.vod.module.musicInfo.LrcContent;
...
public class LrcView extends ScrollView implements  OnTouchListener, ILrcStateContain {
    ...
    class LrcTextView extends TextView {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            //int tempY = (int) height-DisplayUtil.dip2px(mContext, 24); ;//歌词的起点
            //int tempY = DisplayUtil.dip2px(mContext, 10);//歌词的起点
            ...
            switch (lrcState) {
                ...
                case READ_LOC_OK:
                    ...
                    for (int i = 0; i < lrcLists.size(); i++, tempY += textHeight) {
                        if (i == index) {
                            /*// 获得字符串的"长度"
                            float len = currentPaint.getTextSize() * lrcLists.get(i).getLrcStr().length();
                            // 参数color数组表示参与渐变的集合
                            // 参数float数组表示对应颜色渐变的位置
                            int[] a=new int[]{Color.WHITE, Color.BLUE};
                            float[] f=new float[]{float1, float2};
                            Shader shader = new LinearGradient(width / 2-len/2,
                                    tempY,
                                    width / 2+len/2,
                                    tempY,
...
//            pos = (int) (this.getScrollY() -DisplayUtil.dip2px(mContext,9)/ textHeight);
