 ...
 * 本地视屏播放器
 */
public class MusicPlayerActivity extends BaseActivity implements MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener, SurfaceHolder.Callback, View.OnClickListener, SeekBar.OnSeekBarChangeListener {

    private final int BEGINANIMAL = 2;//开始动画
    private final int PROGRESS = 1;//开始进度
    private List<VodMedia> playList;
    VodMedia localCompose;
...
//        sb.setOnSeekBarChangeListener(this);
    ...
    private void getDatas() {
        ...
        name.setText(localCompose.getSongName());
        total = Integer.parseInt(localCompose.getVolume());//总时长
//        krcPath = localCompose.getCompose_other();//歌词地址
        krcPath = MyFileUtil.DIR_KRC.getAbsolutePath();//歌词地址(先写死)
        skipTime = Integer.parseInt(localCompose.getVolume());//前奏时间，为0表示没有跳过前奏
...
//            compose_type = preCompose.getCompose_type();
            compose_type = "1";
            compose_file = preCompose.getPath();
            name.setText(preCompose.getSongName());
            krcPath = preCompose.getPath();
            skipTime = Integer.parseInt("0");//前奏时间
            try {
                handler.removeMessages(PROGRESS);
                nextLrcTv.setText("");
                lastLrcTv.setText("");
    ...
    private void next() {
        ...
        if (nextCompose != null) {
            name.setText(nextCompose.getSongName());
            compose_type = "1";
            compose_file = nextCompose.getPath();
            krcPath = nextCompose.getPath();
            skipTime = Integer.parseInt("0");
            try {
                handler.removeMessages(PROGRESS);
                nextLrcTv.setText("");
                lastLrcTv.setText("");
