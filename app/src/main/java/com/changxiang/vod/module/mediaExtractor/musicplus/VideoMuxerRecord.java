...
public abstract class VideoMuxerRecord {
    ...
    private VideoMuxerRecord(String outputVideo) {
        this.mOutputVideo = outputVideo;
    }

    public final static VideoMuxerRecord createVideoMuxer(String outputVideo) {//合成的文件路i经
     ...
     */
    public abstract void mixRawAudio(File videoFile, File audioFile, File rawAudioFile, int offset, boolean includeAudioInVideo, boolean isRecorderMp3, long allDuration);
    //videoFile 录制的源文件
...
//
    ...
    private static class Mp4Muxer extends VideoMuxerRecord {
        ...
        private boolean getbase = true;//是否获取原本的录音。
        ...
        @Override
        public void mixRawAudio(File videoFile, File audioFile, File rawAudioFile, int offset, boolean includeAudioInVideo, boolean isRecorderMp3, long allDuration) {
...
//			includeAudioInVideo 是否包含视频音轨
            ...
            float audioWeight = 0.3f;//伴奏的音量权重0.3f
            float rawAudioWeight = 0.0f;//录制的音量权重1.8f
            short resultShort;//用于音量权重处理
            short[] outputShortArray;
            LogUtils.sysout("++++++++++++++++录制时长" + duration);
            long longall = (duration / 1000 + 1) * 7 * audioBytesPerSample;//录制时长
            int count = 0;
            this.offsetnum = 96000 * offset / 1000;//1秒的长度为：96000
            if(offset!=0) {
                getbase = false;
            }
            try {
...
//                                extractAudioReadCount = fisExtractAudio.read(extractAudioBuffer);//extractAudioReadCount 音频流数据长度(mp4的音轨)
                                videoAudioReadCount = fisExtractAudio.read(videoAudioBuffer);//extractAudioReadCount 音频流数据长度(mp4的音轨)
                                byte firstByte;
                                byte secondByte;
                                if (videoAudioReadCount != -1) {
                                    extractAudioReadCount = videoAudioReadCount * 2;
//                                        mixAudioBuffer = new byte[mixAudioReadCount];
                                    for (int index = 0; index < videoAudioReadCount; index += 2) {//单通道转双通道
//                                            mixAudioBuffer[2 * t] = RecorderAudioBuffer[t];
//                                            mixAudioBuffer[2 * t + 1] = RecorderAudioBuffer[t];

                                        firstByte = videoAudioBuffer[index];
                                        secondByte = videoAudioBuffer[index + 1];

                                        extractAudioBuffer[2 * index] = firstByte;
                                        extractAudioBuffer[2 * index + 1] = secondByte;
                                        extractAudioBuffer[2 * index + 2] = firstByte;
                                        extractAudioBuffer[2 * index + 3] = secondByte;

                                    }


                                } else {
                                    extractAudioReadCount = videoAudioReadCount;
...
//                                    if(!getbase) {//判断是否使用录音数据
                                    if(false) {//判断是否使用录音数据 f否：
                                        if (this.offsetnum > 0) {////以伴奏为基准，当大于0的时候，表示录音需要加长，小于0，则表示录音需要裁剪掉开头部分。
                                            ...
                                            RecorderAudioReadCount = extractAudioReadCount;
...
//                            DLog.i(TAG, "audio encoder: codec config buffer");
                            LogUtils.sysout("audio encoder: codec config buffer");
...
//                            LogUtils.sysout(String.format(" writing audio sample : size=%s , presentationTimeUs=%s", outBufferInfo.size, outBufferInfo.presentationTimeUs));
//                            DLog.i(TAG, String.format(" writing audio sample : size=%s , presentationTimeUs=%s", outBufferInfo.size, outBufferInfo.presentationTimeUs));
                            if (lastAudioPresentationTimeUs < outBufferInfo.presentationTimeUs) {
                                ...
                                LogUtils.sysout("error sample! its presentationTimeUs should not lower than before.");
...
//                            LogUtils.sysout("*******************presentationTimeUs =" + videoExtractor.getSampleTime());
                        ...
                        if (videoBufferInfo.presentationTimeUs <= duration * 1000) {
...
//                    DLog.i(TAG, "video mix complete.");
                    LogUtils.sysout("video mix complete.");
