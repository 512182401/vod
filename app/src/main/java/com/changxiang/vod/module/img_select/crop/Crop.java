...
public class Crop {
    ...
    static interface Extra {
        ...
        String MAX_X = "max_x";
        String MAX_Y = "max_y";
        String OUTPUT_SIZE="output_size";//输出的图片高宽像素值,这里只支持正方形的裁切,所以只需要一个参数
        String ERROR = "error";
    }
    ...
    /**
     * 输出的图片高宽像素值,这里只支持正方形的裁切,所以只需要一个参数
     * @param size 输出的图片的最大高宽值;小雨则按实际大小输出
     * @return
     */
    public Crop outputSize(int size){
    	 cropIntent.putExtra(Extra.OUTPUT_SIZE, size);
    	 return this;
    }
     ...
     */
    public Crop asSquare(int ASPECT_X,int ASPECT_Y ) {
        cropIntent.putExtra(Extra.ASPECT_X, ASPECT_X);
        cropIntent.putExtra(Extra.ASPECT_Y, ASPECT_Y);
        return this;
    }
    ...
    public void start(Activity activity, int w, int h) {
        activity.startActivityForResult(getIntent(activity), REQUEST_CROP);
    }
    ...
    public static void pickImage(Activity activity) {
        ...
        try {
            ...
            LogUtils.sysout( "pickImageREQUEST_PICK" );
        } catch (ActivityNotFoundException e) {
            LogUtils.sysout( "pickImage+++++++++++REQUEST_PICK" );
